...
function StepItem({
  ...
  return (
    <div className="group flex items-center gap-2.5 py-1.5 px-2 rounded-lg hover:bg-gray-50 transition-colors">
      ...
      <button
        onClick={onDelete}
        className="opacity-0 group-hover:opacity-100 p-0.5 text-gray-400 hover:text-red-500 transition flex-shrink-0"
      >
  ...
  return (
    <section
      className={`
        ...
        ${todo ? 'translate-x-0' : 'translate-x-full pointer-events-none'}
      `}
    >
      {todo && (
        <>
          ...
          <div className="flex-1 overflow-y-auto p-6 space-y-6">
            ...
            <div className="flex items-start gap-4">
              <button
                ...
                className={`mt-1 flex-shrink-0 transition-colors ${todo.completed ? 'text-indigo-500' : 'text-gray-300 hover:text-indigo-500'}`}
              >
              ...
              <textarea
                ...
                className={`w-full text-xl font-bold bg-transparent border-none focus:ring-0 resize-none overflow-hidden transition-colors ${todo.completed ? 'text-gray-400 line-through' : 'text-gray-800'}`}
                rows={2}
              ...
              <div className="flex gap-2">
                {PRIORITIES.map((p) => (
                  <button
                    ...
                    className={`px-3 py-1.5 text-sm rounded-lg border transition-all ${priorityButtonClass(todo.priority === p, p)}`}
                  >
                    {priorityLabel(p)}
                  </button>
                ))}
